...
function ExportDatasetModal(): JSX.Element {
    ...
    /*
        author : minguin
        date : 2021.10.07
        comments : 출력된 모달 창의 Export Format을 CVAT for 000 1.1로 지정(Defualt)
        Original source : (#61) form.setFieldsValue({ selectedFormat: 'CVAT for video 1.1' });
        (#63) form.setFieldsValue({ selectedFormat: 'CVAT for images 1.1' });
    */
    ...
    const initActivities = (): void => {
        if (instance instanceof core.classes.Project) {
            ...
            if (instance.mode === 'interpolation' && instance.dimension === '2d') {
                form.setFieldsValue({ selectedFormat: 'COCO 1.0' });
    ...
    const test = `${instance?.name}`.split('_');
